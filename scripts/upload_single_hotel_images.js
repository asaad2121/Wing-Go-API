...
const ERROR_LOG_PATH = path.join(__dirname, 'upload_errors_single.log');
const HOTEL_ID = 59;
...
const seedImagesForSingleHotel = async () => {
    const hotel = await models.Hotels.findByPk(HOTEL_ID);
    if (!hotel) {
        console.error(`Hotel with ID ${HOTEL_ID} not found.`);
        return;
    }

    console.log(`Seeding images for: ${hotel.name} (ID: ${HOTEL_ID})`);
    ...
    console.log(`Finished uploading for ${hotel.name}`);
};

seedImagesForSingleHotel();
