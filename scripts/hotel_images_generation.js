// node scripts/hotel_images_generation.js --start=140
...
const ERROR_LOG_PATH = path.join(__dirname, 'upload_errors.log');

// Get CLI argument --start=N
...
const seedHotelImages = async () => {
    const hotels = await models.Hotels.findAll({ order: [['id', 'ASC']] });

    const slice = hotels.slice(startIndex, startIndex + batchSize);
    console.log(`🚀 Processing hotels ${startIndex + 1} to ${startIndex + slice.length}`);

    for (const hotel of slice) {
        const query = `${hotel.name} hotel exterior`;
        ...
        for (let i = 0; i < imagesToUpload.length; i++) {
            ...
            const publicId = generatePublicId(hotel.name, i + 1);

            console.log(`📷 Uploading image ${i + 1} for ${hotel.name}`);

            const cloudData = await uploadImageToCloudinary(imageUrl, publicId);
            if (!cloudData) {
                fs.appendFileSync(ERROR_LOG_PATH, `${hotel.name} | ${imageUrl} | Upload Failed\n`);
                continue;
            }

            try {
                await models.HotelImages.create({
                    hotelId: hotel.id,
                    imagePublicId: cloudData.public_id,
                    format: cloudData.format,
                });

                console.log(`Image ${i + 1} saved for ${hotel.name}`);
            } catch (dbErr) {
                fs.appendFileSync(ERROR_LOG_PATH, `${hotel.name} | ${cloudData.url} | DB Insert Failed\n`);
                console.error(`DB insert failed for ${hotel.name} →`, dbErr.message);
            }
        }
    }
